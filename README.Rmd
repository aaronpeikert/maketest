...
The [Makefile](Makefile) is the heart of the project, the rest is just a silly example (multipling numbers by 2). It is farely well commented, but it should be sufficient to just edit the variables at the beginning and of course add you own scripts. Please note that the target `input` must be build --- in a seperate call to make ---  before targets based on it. I have not (yet) figured out how to circumvent this inconvinience.
